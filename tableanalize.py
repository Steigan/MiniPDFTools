        ...
        for i, gl in enumerate(guidelines):
            if self.start_idx == -1: # начало еще не привязано
                if i == 0 and self.start_coord <= gl: # начало приходится на начальный край или находится до него
                    self.start_idx = i
                elif self.start_coord >= prev_coord and self.start_coord <= gl: # начало между предыдущей направляющей и текущей
            ...
            if self.start_idx > -1 and self.end_idx == -1: # конец еще не привязан
                if i == max_gl_idx and self.end_coord >= gl: # конец приходится на конечный край или находится за ним
                    self.end_idx = i
                elif self.end_coord >= prev_coord and self.end_coord <= gl: # конец между предыдущей направляющей и текущей
...
def parse_page_tables(page, worksheet, start_row, cell_format, strong:bool=True):
    ...
    vert_guideline_map = {} # Типа карта перевода горизонатальных координат в индекс соответствующей вертикальной направляющей
    last_cc = -100
    gl_idx = -1
    for it in s_vert:
    ...
    hori_guideline_map = {} # Типа карта перевода вертикальных координат в индекс соответствующей горизонтальной направляющей
    last_cc = -100
    gl_idx = -1
    for it in s_hori:
        if it - last_cc > max_delta:
        ...
        for bd in hori_borders:
            ...
            if m_idx >= 0: # если у этого горизонтального отрезка есть начало
                while m_idx <= bd.end_idx: # обходим все возможные точки пересечения с перпендикулярными направляющими
                    ...
                    for cross_bd in vert_borders: # проверяем все перпендикулярные направляющие на пересечение в текущей точке
                        if cross_bd.guideline_idx == m_idx: # есть вероятность пересечения
                            if cross_bd.start_idx == bd.guideline_idx:
                                nodes[bd.guideline_idx][cross_bd.guideline_idx] |= hor_dir | NODE_DIR_DOWN # в этом узле есть пересечение
                            elif cross_bd.end_idx == bd.guideline_idx:
                                nodes[bd.guideline_idx][cross_bd.guideline_idx] |= hor_dir | NODE_DIR_UP # в этом узле есть пересечение
                            elif cross_bd.start_idx < bd.guideline_idx and bd.guideline_idx < cross_bd.end_idx:
                                nodes[bd.guideline_idx][cross_bd.guideline_idx] |= hor_dir | NODE_DIR_DOWN | NODE_DIR_UP # в этом узле есть пересечение
                    m_idx += 1
                ...
                if (node & NODE_DIR_RIGHT) and (node & NODE_DIR_DOWN): # найден стартовый узел
                    for cdx2 in range(cdx + 1, len(vert)): # обходим следующие узлы по горизонтали
                        if (nodes[rdx][cdx2] & NODE_DIR_DOWN): # найден верхний правый узел
                            for rdx2 in range(rdx + 1, len(hori)):  # обходим следующие узлы по вертикали по правой стороне
                                ...
                                if (nodes[rdx2][cdx2] & NODE_DIR_LEFT): # найден нижний правый узел
                                    ...
                                    if (rdx2 > rdx + 1) or (cdx2 > cdx + 1):
                                        worksheet.merge_range(start_row + rdx, cdx, start_row + rdx2 - 1, cdx2 - 1, recttext, cell_format)
                                    else:
